 ...
 * 声明baidu包
 */
var baidu = baidu || {version: "1-3-1"}; // meizz 20100513 将 guid 升级成 \x06
 ...
 * 
 * 但是这些绑定函数的执行顺序无法保证。
 * 处理会调用通过addEventListener绑定的自定义事件回调函数之外，还会调用
 * 直接绑定到对象上面的自定义事件。例如：
 ...
 * @param {Object} options optional     其他可选项
 ...
 * @param {object} options 元素创建时的选项，具体如下：
 ...
 * 查询一个元素是否包含指定的属性
 * 
 * @param {string|DOMElement} element 元素的id或DOM元素
 ...
 *
 * @remark 传入多个class时，只要其中有一个class不在当前元素中，则添加所有class，否则删除所有class。
 * 
 */
 ...
 * 
 * path: baidu/dom/get.js
 * author: berg
 * version: 1.0.0
 * date: 2010-07-12  季度大会前一天
 */
...
(function(){
    var proto = {},
        isArray = baidu.lang.isArray,
        isElement = baidu.lang.isElement,
    ...
    /**
     * 包装each方法，方便用户自行遍历dom
     * @example : baidu.e(baidu.dom.query('div')).each(function(){
     *      //each div
     * });
     * @param {function} iterator 遍历时的调用方法
     */
    proto['each'] = function(iterator){
        //每一个iterator接受到的都是封装好的node
        baidu.object.each(this, function(node){
            iterator.call(this, (new Element(node)));
        });
    };

    //包装dom包下的大部分方法，除了drag和ready
    baidu.each(("addClass draggable empty hide show insertAfter insertBefore insertHTML removeClass " + 
              ...
              "last next prev contains getAttr getPosition getStyle g q query hasClass intersect remove").split(' '),
              function(fnName){
                  proto[fnName] = proto[fnName.replace(/^get[A-Z]/g, stripGet)] = wrapFn(fnName, "dom");
              });

    //包装event中的on 和 un
    baidu.each(("on un").split(' '), function(fnName){
        proto[fnName] = wrapFn(fnName, "event");
    });
    //包装event的快捷方式
    ...
    function wrapFn(fnName, fn){
        return function(){
            ...
            baidu.object.each(this, function(item, key){
                //把节点放到参数串的第一位
                Array.prototype.unshift.call(args, item);
                //包装on, un方法中传递的event对象
                //todo: 由于on, un方法中已经对handler做了包装，暂时无法再次封装
                /*
                 *if(packageName == 'event'){
                 *    var orgFn = args[2];
                 *    args[2] = function(e){
                 *        orgFn.call(this, baidu.event.get(window.event || e));
                 *    }
                 *}
                 */
                ...
                //参数还原，供下次调用使用
                Array.prototype.shift.call(args, item);
            });
            //如果原方法返回的是element或者element array，则包装一下，让用户能继续点下去
            //否则直接返回原方法的返回值，供用户使用
            return (isElement(ret) || (isArray(ret) && isElement(ret[0]))) ? new Element(ret) : ret;
        }
    }
     ...
     *
     * @usage
     * var d = baidu.element(baidu.dom.query(".class"));
     *
     * d.addClass("myClass").show();
     */
    baidu.e = baidu.element = function(node){
        return new Element(baidu.g(node));
    };
     ...
     * @example baidu.e.extend({
     *  "moveBy" : baidu.fx.moveBy
     * });
     */
    baidu.element.extend = function(json){
        baidu.object.each(json, function(item, key){
            Element.prototype[key] = wrapFn(key, item);
        });
    };

})();

/// -----  不直接把所有dom包下东西都包含，用户要用哪个方法自己选
