...
var ReportUIChange = {
	...
	reportTypeChange: function(e) {
		...
		} else if ((lastReportType == "Rr10") && (!G("Rr10").checked)){
			//根据记录的状态读取分日复选框
...
function buildReport(data) {
	if (rightValidate(data)) {
		...
		setFlash();
		//记录信息
		//~ if (G("Rr10").checked && tableData.length != 0) {
			//~ //获取关键词状态
			//~ var startRecord = numPerPage * ( p._nowPage - 1 );
			//~ var endRecord = startRecord + numPerPage;
			//~ QUERY_REPORT_RENDER.loadKeywordStatus(startRecord, (endRecord > tableData.length) ? tableData.length : endRecord);
			
			//~ QUERY_REPORT_RENDER.loadKeywordStatus();
			//~ FC_GE_LOG.send({
...
function getReportData(page) {
	if (tableData.length > 0) {
		...
		if (dataTmp.length > 0) {
			...
			if(nowReportParam.reportType == 6){  //当是搜索词报告时，总计列前补充一空列,同时修改startIndex
				var _nd = C('td');
	            _nr.appendChild(_nd);
	            _nd.align = 'left';
	            _nd.innerHTML = '';   
			...
			if(tb.tbody.rows.length <= FOOT_TITLE_MIN_ROWS){	
				...
				for(var i = startIndex; i < colsLength; i++){	//跳过第一列
					var _td = C('td');
					_nr.appendChild(_td);
	...
	hide(G("queryTableHeadDown"));
	
	//增加对搜索词报告的数据回传处理
	//author luoyujia@baidu.com
	//当表格中无数据时不发送请求，此时startRecord和endRecord均为undefined。还有可能是endRecord为0时，也不作处理。
	if (nowReportParam.reportType == 6 && endRecord) {
		...
		//获取关键词状态
		QUERY_REPORT_RENDER.loadKeywordStatus(startRecord, endRecord);
		...
		G("queryKRUp").onclick = QUERY_REPORT_RENDER.addwordsKR;
		G("queryKRDown").onclick = QUERY_REPORT_RENDER.addwordsKR;
	}
}
		...
		case reportUnitList[2][4][1]:	//粒度：创意
			if (nowReportParam.reportType == 6) {//类型：搜索词报告
				...
				if (queryPlanId >= 0 && queryUnitId >= 0) {
					...
					//同时判断当前计划或单元是否已被删除。
					if ((t.parentNode == tb.tbody.rows[_index % numPerPage].childNodes[queryPlanId]) && (!t.innerHTML .match(/已.*删.*除/))) {//用户选择的是推广计划
						G("SpaceVenusLink").innerHTML = "推广计划";
						G("SpaceVenusLink").href = VENUS_HOST + "unit/unitListAction.do?userid=" + spaceLinkData.accountid + "&planid=" + spaceLinkData.planid;
					} else if(!t.innerHTML .match(/已.*删.*除/)) {
						G("SpaceVenusLink").innerHTML = "推广单元";
						G("SpaceVenusLink").href = VENUS_HOST + "material/materialListAction.do?userid=" + spaceLinkData.accountid +"&planid=" + spaceLinkData.planid + "&unitid=" + spaceLinkData.unitid;
					} else {
						//计划或单元已删除，直接返回。
...
function setFlashItem(){
	...
	if (getColumnIndex('展现量') != -1){
		html += '<input type="checkbox" name="reportItem" value="展现量" id="itemShow"' + ((nowReportParam.reportType == 6) ? 'checked="true"' : '') + '/><label for="itemShow">展现量</label>';
	}
...
function setPlan(d) {
	if (typeof d == 'string') {
		return insertWbr(escapeHTML(d));
	} else {
		//增加推广计划渲染对搜索词报告的支持
		if ((nowReportParam.mtldim == reportUnitList[2][1][1] || nowReportParam.reportType == 6) && IS_PURE_FENGCHAO) {
			var tmp = [];
			tmp[tmp.length] = '<a href="#" onmouseout="spaceOut(event)" onmouseover="spaceHover(event)" onclick="spaceReportLink();return false">';
			tmp[tmp.length] = insertWbr(escapeHTML(d[1]));
			tmp[tmp.length] = '</a>';
			return tmp.join("");
		} else {
			return insertWbr(escapeHTML(d[1]));
		}
	}	
}
...
var QUERY_REPORT_RENDER = {
	 ...
	 *重新获取关键词状态
	 */
	loadKeywordStatus : function(startRecord, endRecord) {
		var queryString = [];
		for (var i = startRecord; i < endRecord; ++i) {
			queryString[queryString.length] = tableData[i][0];
		}
	//~ loadKeywordStatus : function() {
		//~ var queryString = [];
		//~ var len = tableData.length;
		//~ for (var i = 0; i < len; ++i) {
			//~ queryString[queryString.length] = tableData[i][0];
		...
		mtlDWR.getQueryWordStat(USER_ID, queryString.join(";"), {
			callback : function(responseText) {
				...
				//~ QUERY_REPORT_RENDER.handleWordStateChange(responseText);
			}
		});
	},
	...
	//~ handleWordStateChange : function(responseText) {
		...
		var cellData = responseText.split(","), 
			len = startRecord + cellData.length,
			actionColumnIndex = getColumnIndex("操作"),
			tmp;
		for (var i = startRecord; i < len; ++i) {//重新渲染操作列
			QUERY_REPORT_RENDER.reRenderCell(i, actionColumnIndex, cellData[i % numPerPage]);
	...
	handleAddKWCallBack : function(responseArray) {//重新显示页面内容
		...
		for(var i in QUERY_REPORT_RENDER.floatRowData) {//每条数据的格式为{data, failed, message, least}
			...
			tmpData[i].failed = (responseArray[index][0] != "true");
			//构造信息。如果添加成功，则信息为：已添加！出价XX元
			//如果低于最低展现价格，则信息为：添加成功，出价XX元，最低展现价格为XX元
			//如果添加失败，则直接从LANG.QUERY中读取错误信息，或者直接读取回传数据中的错误信息
			tmpData[i].message = (LANG.QUERY[responseArray[index][0]]) ?//失败，从LANG.QUERY中读取错误信息
				LANG.QUERY[responseArray[index][0]] : (tmpData[i].failed ?//失败，直接读取错误数据
					responseArray[index][1] : (responseArray[index][2] ?//成功，但是低于最低展现价格
						('添加成功，出价' + responseArray[index][1] + '元') : //直接成功
						('已添加！出价' +  + responseArray[index][1] + '元')));
			flag = !tmpData[i].failed && responseArray[index][2];
			tmpData[i].least = flag ? responseArray[index][2] : undefined;
			tmpData[i].winfoid = flag ? responseArray[index][3] : undefined
			++index;
			//统计成功单词数
			if (!tmpData[i].failed) {
				++succeedItems;
	...
	queryBanchHandler : function(e) {
		...
		var floatData = {},//批量操作时，传入数据为一个哈希表
			...
			rowIndex,
			startRecord = numPerPage * ( p._nowPage - 1 );
		for (var i = 0; i < len; ++i) {
			rowIndex = startRecord + (checkedItems[i].id.match(/\d+/)[0] - 0);
	...
	buildCol : function(tar) {
		...
		tar[tar.length] = {"title": "操作","align": "left", "width":"142px", "render": QUERY_REPORT_RENDER.renderQueryStatus};
	},
	...
	renderCheckBox : function(d) {
		...
		return '<input id="queryCheck' + (d % numPerPage) + '" type="checkbox"/>';
	},
	...
	renderQueryStatus : function(d, rowIndex) {
		...
		//第一次载入页面或页面最大显示个数发生更改时直接返回提示文字
		if ((d == "" || (rowIndex == undefined))) {
			return "处理中...";
		}
		var tmp = [];
		tmp[tmp.length] = '<div id= "qaDiv' + (rowIndex % numPerPage) + '" style="height:21px;" class="queryUsable ';
	...
	_disableCheck : function(index) {
		...
		node.disabled = true;
		node.checked = false;
		tb.table.rows[((index - 0) + 1) % numPerPage].className="";
	},
	...
	showQueryNegative : function(rowIndex) {
		...
		var node = G("qaDiv" + rowIndex % numPerPage);
		node.style.height = (node.style.height == "21px") ? "42px":"21px";
	},
	...
	showFloat : function(status, rowData) {
		...
		if ((status != 0) && (len != 1)) {
			tmp[tmp.length] = "批量添加关键词";
		} else {
			tmp[tmp.length] = msg[status][0 + offset];
		}
	...
	linkToPasvKR : function(index) {
		...
		var refer = (getLength(QUERY_REPORT_RENDER.floatRowData) == 1) ? 'queryReportAddWord' : 'queryReportAddWordBatch',
	...
	modifyKwBid : function(index) {
		...
		var rowData = QUERY_REPORT_RENDER.floatRowData[index].data[0].split(","),
			form = Cform(VENUS_HOST + 'material/wordOpAction.do?userid=' + USER_ID + '&winfoid=' + QUERY_REPORT_RENDER.floatRowData[index].winfoid);
		form.target = "_blank";
	...
	queryMsg : [
		//添加否定关键词
		["添加为否定关键词", "您要添加为单元级别的否定关键词有", '个：<br><span class="queryGrayer qfaHeadMsg">以下词被搜索到后将不触发您的推广结果。</span>', "推广单元：", "推广计划："],
		//单独添加关键词
		["添加为关键词", "您要添加的关键词有", "个：", "所属推广单元：", "所属推广计划："],
		//添加关键词成功和添加否定关键词成功将共享这一组状态字，因为页面中的其他内容二者是一致的；通过偏移量决定界面显示内容。
		["添加为关键词", "以下", "个关键词已添加：", "所属推广单元：", "所属推广计划：", "个关键词未成功：", "添加否定关键词", "以下", "个关键词已添加为否定关键词：", "所属推广单元：", "所属推广计划：", "个关键词未被添加为否定关键词：", "批量添加关键词"],
	],
	...
	queryFloatAction : [
		...
		//添加关键词
		'<div class="queryPositive"><p style="padding:0;display:inline;margin-right:2px;">出价：</p>\
	...
	queryStatus : {
		...
		"0" : '"><a class="queryUsable" id="queryActionButton" onmouseover=\'javascript:this.className=\"queryUsableOver\";\' onmouseout=\'javascript:this.className=\"queryUsable\";\'></a>\
				<a id="queryActionPos" class="qalabel" href="javascript:;">添加为关键词</a>\
				<a id="queryActionNeg" class="qalabel qalabelBorder" href="javascript:;">添加为否定关键词</a>\
			</div>',
...
function domReady() {
	...
	//~ window.p = new Pagination("Page", 1, 1, 1, 2, function(page) {
		//~ getReportData(page);		
		//~ if (nowReportParam.reportType == 6) {
			//~ //获取关键词状态
		//~ var startRecord = numPerPage * ( p._nowPage - 1 );
...
window.onload = function(){
	...
	//页面导入之前禁止生成报告
	G("BuildReportBtn").disabled = false;
}
...
AdaptiveSelect.prototype = {	
	...
	init:function(){
		...
		this.showOptions = function () {			
			...
			autoWidth = autoWidth < _this._originalSelect.offsetWidth ? _this._originalSelect.offsetWidth : autoWidth;		//自适应宽度：获取div宽度即可
