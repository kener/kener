...
window.onload = function(){
	...
	new AdaptiveSelect(rs1,10,123,200);		//对象，下拉列最大条数，定宽，监控时间
	new AdaptiveSelect(rs2,10,123,200);		
	new AdaptiveSelect(rs3,10,123,200);		
}
...
function AdaptiveSelect(objSelectBox,iMaxRow,iSelectWidth,iMonitorInterval){
	...
	//外部可选配置参数初始化
	this.awMaxHeight = ((iMaxRow < 0) ? 10 : iMaxRow) * this.lineHeight; 				//默认下拉选项最大高度
	this.awWidth = (iSelectWidth < 0) ? objSelectBox.style.width : iSelectWidth; 	//选项宽度
	this.awMonitorInterval = (iMonitorInterval < 0) ? 200 : iMonitorInterval;		//间隔时间
	...
	this.lineHeight = 15; 								//默认行高,需要同时修改相应1的CSS
...
AdaptiveSelect.prototype = {	
	...
	init:function(){
		...
		textInput.onmouseover = function(e){
			G(_this.awArrowIDBase).className='awArrow awArrowHover';
			_this.isInside = 1;
		};
		//选项：鼠标移出，改变样式
		textInput.onmouseout = function(e){
			G(_this.awArrowIDBase).className='awArrow';
			_this.isInside = 0;
		};
		//选项：失去焦点，恢复样式
		...
		this.showOptions = function () {			
			...
			for(var j = 0; j < optLength; j++) {
				...
				addEvent(optLink, 'click',function(event){onOptClick(event)});				//绑定列表鼠标点击响应
				addEvent(optLink,'mouseover',function(event){onOptLinkMouseOver(event)});	//绑定列表鼠标移动响应
				//禁止用户选择
				optLink.onmousedown = function() { 
					return false; 
				}; 	
				optLink.onselectstart = function() { 
					return false; 
				}; 					
			}	
			...
			var autoWidth = _this._originalSelect.offsetWidth;											//自适应宽度：默认包含滚动条宽度18px
			autoWidth = (autoWidth>_this.awWidth?autoWidth:_this.awWidth+19);							//自适应宽度：如果比指定短用指定宽度
			_this._originalSelect.style.display = 'none';
			var autoHeight = optGroup.offsetHeight-2;													//自适应高度：包括上线2px边框				
			if (autoHeight>_this.awMaxHeight){															//超出指定最高需要调整
				...
				setTimeout("javascript:var optGroup = G('"+_this.awOptGroupIDBase+"');optGroup.scrollTop = '"+
				(linkNum*_this.lineHeight-Math.floor(_this.awMaxHeight/_this.lineHeight/2)*_this.lineHeight)+"'",10); //定位显示
			}
			else{
				optGroup.style.width = autoWidth-18+'px';												
				optGroup.style.height = autoHeight+'px';
				optIframe.style.width = autoWidth-16+'px';
				optIframe.style.height = autoHeight+'px';
			}
			...
			onOptClick = function(e){
				var tempText = _this._originalSelect.options[(e.target || e.srcElement).id].value;		
				_this._originalSBox.setSelect(tempText);
				var mySelectText = G(_this.awSelectTextIDBase);
			...
			//选项Link：移动鼠标，改变样式
			onOptLinkMouseOver = function(e){
				var linkNum = (e.target || e.srcElement).id;
		...
		this.hideOptions = function(){
			var optGroup = G(_this.awOptGroupIDBase);
			var optIframe = G(_this.awOptIframeIDBase);
			if (optGroup != null){
