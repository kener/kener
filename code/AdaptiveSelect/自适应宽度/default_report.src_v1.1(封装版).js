...
var ReportUIChange = {
	...
	reportTypeChange: function() {
		...
		if (e) {
			...
			if (G("Rr10").checked && (target ? (target.tagName != "LABEL") : true)) {
				//记录上次复选框状态
...
function getReportData(page) {
	...
	G("queryTableHead1").style.display="none";
	G("queryTableHead2").style.display="none";
		...
		mtlDWR.getQueryWordStat(USER_ID, queryString.join(";"), {
			callback : function(responseText) {
				...
				for (var i = startRecord; i < len; ++i) {//重新渲染操作列
					...
					if (cellData[i] != 0) {//不可被添加为关键词时，更改表格数据
						//将复选框列置灰。
						G("queryCheck" + i).disabled = true;
						G("queryCheck" + i).checked = false;
					}
				}
			}
		});
		
		qbaButton_1 = new Button("qbaButton1", "添加为关键词", "rcbtn3", "rcbtn3_disabled", QUERY_REPORT_RENDER.queryBanchHandler, 1);
		qbaButton_2 = new Button("qbaButton2", "添加为关键词", "rcbtn3", "rcbtn3_disabled", QUERY_REPORT_RENDER.queryBanchHandler, 1);
		checkList = new CheckList();
		
		G("queryTableHead1").style.display="";
		G("queryTableHead2").style.display="";
		...
		case reportUnitList[2][4][1]:	//粒度：创意
			if (nowReportParam.reportType == 6) {//类型：搜索词报告
				...
				if (queryPlanId >= 0 && queryUnitId >= 0) {
					...
					//同时判断当前计划或单元是否已被删除。
					if (t.innerHTML == tableData[_index][queryPlanId][1] && t.innerHTML .indexOf("已删除") == -1) {//用户选择的是推广计划
						G("SpaceVenusLink").innerHTML = "推广计划";
						G("SpaceVenusLink").href = VENUS_HOST + "plan/planListAction.do?userid=" + spaceLinkData.accountid;
		...
		if (j != 0 && flashData[j - 1][dateColumn] == flashData[j][dateColumn]) {
			...
			if(gap > 1){
				for(var y = gap - 1; y > 0; y--){
					...
					flashData.splice(j, 0, [dateToString(date,'YYYY-MM-DD')].concat(emptyDateAdapter));
				}
			}
		}
	}
...
var QUERY_REPORT_RENDER = {
	...
	tableClickHandler : function(e) {
		...
		tarId = target.id;
		if (tarId.match("queryAction")) {//表格中的操作列被点击。此处，只有表格中的操作列的id值满足条件。
	...
	addwordsKR : function() {
		...
		var nf = Cform(MERCURY_HOST + 'spread/InitAddWordsKR.do?userid=' + USER_ID + '&actv=true&refer=queryReportLink', 'post');
	...
	queryBanchHandler : function(e) {
		...
		for (var i = 0; i < len; ++i) {
			...
			uniqueId = tableData[rowIndex][0].toLowerCase();
			if (QUERY_REPORT_RENDER.queryMap[uniqueId]) {//发现重复元素，缓存
				QUERY_REPORT_RENDER.queryMap[uniqueId] += "," + rowIndex;
				QUERY_REPORT_RENDER.queryMap.repeat = true;
			} else {
				QUERY_REPORT_RENDER.queryMap[uniqueId] = rowIndex;
				//获取存储于checkBox的id中的行索引数据并填充到floatData中。
				floatData[rowIndex] = tableData[rowIndex];
			}
	...
	reRenderCell : function(rowIndex, colIndex, cellData) {
			...
			tb.tbody.rows[rowIndex].children[colIndex].innerHTML = 
	...
	renderQueryStatus : function(d, rowIndex) {
		...
		var tmp = [];
		tmp[tmp.length] = '<div id= "queryAction' + rowIndex + '" style="height:20px;" class="queryUsable ';
	 ...
	 *搜索词报告弹出框鼠标点击事件处理函数
	 */
	queryFloatMsgHandler : function(e) {
		
		var e = e || window.event;
		target = e.target || e.srcElement;
		tagName = target.tagName.toLowerCase();
		//当浮出层上的“删除”、“更多推荐”等超链接被点击
		//注意，为了简便起见在此使用了一个略带硬编码的策略：所有超链接的value值均与其对应的处理函数名相匹配。
		if (tagName == "a") {
			...
			if (target.id.match("queryFloatConfirm")) {//确定，发送数据
				...
				//组装和记录查询列
				var queryString = [];
				//floatRowDatas可能为一个哈希表。
				for (var i in QUERY_REPORT_RENDER.floatRowDatas) {
					//~ if (i == "length") {
						//~ continue;
					//~ }
					queryString[queryString.length] = QUERY_REPORT_RENDER.floatRowDatas[i][0];
				}
				
				if (target.id == "queryFloatConfirmPos") {//添加关键词
					//出价信息
					var bid = (G("qfaSelfBid").checked)? (parseInt(G("selfBidInput").value)) 
						: ((G('qfaUnitBid').checked)? 0 : -1);
					MarsWordMgr.addKeywordsFromQueryReport(USER_ID, queryString.join(";"), bid, G("qfaSelect").value, {
						callback : function(responseArray) {//重新显示页面内容
							//全局错误处理
							...
							//计数器
							index = 0;
							for(var i in QUERY_REPORT_RENDER.floatRowDatas) {//每条数据的格式为{data, failed, message, least}
								//数据回传时可保证QUERY_REPORT_RENDER.floatRowDatas为纯数据
								//~ ++tmpData.length;
								tmpData[i] = {};
								tmpData[i].data = QUERY_REPORT_RENDER.floatRowDatas[i];
								tmpData[i].failed = (responseArray[index][0] == "false")?true : false;
					...
					MarsWordMgr.addNegativesFromQueryReport(USER_ID, queryString.join(";"), G("qfaNegSelect").value, {
						callback : function(responseArray) {
							...
							floatWindowShow("queryFloatWait",0);
							//构造新的浮出层显示
							var tmpData = {}, i, floatData = {};
							//取出存在于缓冲区的唯一数据
							for (i in QUERY_REPORT_RENDER.floatRowDatas) {
								break;
							}
							tmpData.data = QUERY_REPORT_RENDER.floatRowDatas[i];
				...
				QUERY_REPORT_RENDER.queryFloatClose();
			}
			if (target.id != "queryFloatConfirmPos") {//除了”确定“之外的其他按钮点击均会清空去重缓存
				//重置缓存区
	...
	queryFloatClose : function() {
		...
		for (var i in QUERY_REPORT_RENDER.floatRowDatas) {
			//~ if (i == "length") {//当遍历至length元素时跳过
				//~ continue;
			//~ }
			if (!QUERY_REPORT_RENDER.floatRowDatas[i].failed) {//（否定）关键词添加成功
				//重新渲染操作列
				QUERY_REPORT_RENDER.reRenderCell(i, 
					actionColumnIndex, 
					(QUERY_REPORT_RENDER.floatRowDatas[i].negative? //未被添加为关键词
						((QUERY_REPORT_RENDER.floatRowDatas[i].message.match("精确"))? "3" : "2") : //读取匹配模式
						"1"));
				QUERY_REPORT_RENDER._disableCheck(i);
			} else if (QUERY_REPORT_RENDER.floatRowDatas[i].message.match("被删除")) {//单元或计划已被删除
				QUERY_REPORT_RENDER.reRenderCell(i, actionColumnIndex, "-1");
				QUERY_REPORT_RENDER._disableCheck(i);
			}
		}
		//重复元素判断
		var repeatRows, status, i, j, len;
		if (QUERY_REPORT_RENDER.queryMap.repeat) {
			for (i in QUERY_REPORT_RENDER.queryMap) {
				if (i == "repeat") {
					continue;
				} 
				repeatRows = QUERY_REPORT_RENDER.queryMap[i].split(",");
				//根据第一个重复元素重新渲染重复列
				status = tableData[repeatRows[0]][actionColumnIndex];
				len = repeatRows.length;
				for (j = 1; j < len; ++j) {
					QUERY_REPORT_RENDER.reRenderCell(repeatRows[j], actionColumnIndex, status);
					if (status != "0") {//如果不可继续添加，则禁用复选框。
						QUERY_REPORT_RENDER._disableCheck(repeatRows[j]);
					}
				}
			}
		}
	...
	_disableCheck : function(index) {
		...
		//将复选框置灰
		G("queryCheck" + index).disabled = true;
		G("queryCheck" + index).checked = false;
	},
	
	/**
	 *搜索词报告弹出框键盘事件处理函数
	 */
	queryFloatActionHandler : function(e) {
		
		var e = e || window.event;
		target = e.target || e.srcElement;
		//添加关键词时自定义出价文本框触发事件
		if (target.id == "selfBidInput") {			
			//将提示文本内容置空
			G("selfBidTip").innerHTML = "";
	...
	showQueryNegative : function(rowIndex) {
		...
		var node = G("queryAction" + rowIndex);
		node.style.height = (node.style.height == "20px")?"40px":"20px";
	},
	...
	showFloat : function(status, rowData) {
		...
		//传入的rowData可能为两种格式：一种只包含纯数据，另一种为一个对象数组（或者一个哈希表）。需要对两种情况分别处理
		QUERY_REPORT_RENDER.floatRowDatas = rowData;
		//当rowData不是数组时计算rowData的行数
		var tmp, 
		...
		var offset = (rowData[randData?randData : 0].negative) ? 6 : 0, //添加否定关键词成功时偏移量为6
		tmp = [];
		...
		for (var i in rowData) {
			...
			tmpItem = QUERY_REPORT_RENDER.buildMessageItem(i, msg[status], offset, ((len > 1)?"取消" : undefined));
			if (!rowData[i].failed) {
		...
		if(successItems.length != 0) {
			//成功信息
			tmp[tmp.length]  = QUERY_REPORT_RENDER._buildMsgArea(msg[status][1], 
				msg[status][2 + offset] + //是否显示重复信息
					((QUERY_REPORT_RENDER.queryMap.repeat) ?  QUERY_REPORT_RENDER.repeatMsg : ""),
				successItems);
		}
		//添加失败关键词列表
		...
		//添加事件处理函数
		G("qfBody").onclick = QUERY_REPORT_RENDER.queryFloatMsgHandler;
		G("qfBody").onkeyup = QUERY_REPORT_RENDER.queryFloatActionHandler;
	...
	_buildMsgArea : function(head1, head2, items) {
		...
		tmp[tmp.length] = '</div><div class="queryMsgArea ">';
	...
	buildMessageItem : function(index, msg, offset, cancelButton) {
		...
		var rowData = QUERY_REPORT_RENDER.floatRowDatas,
		result = [],
		//如果rowData中仅存在行数据（而没有更多的信息，此时为前端即将发往服务器的数据），则该值为true。
		pureData = (rowData[index].data == undefined);
		result[result.length] = '<div id="queryItem' + index + '">';
		//判断是否需要对每一项数据的“取消”按钮
		if(cancelButton && pureData) {
		...
		result[result.length] = (pureData ? rowData[index][getColumnIndex("搜索词")] : rowData[index].data[getColumnIndex("搜索词")]);
		result[result.length] = '</b></div><div class="queryMsgItem"><span class="queryGrayer">';
		result[result.length] = msg[4 + offset] + '</span>';
		result[result.length] = (pureData ? rowData[index][getColumnIndex("推广计划")][1] : rowData[index].data[getColumnIndex("推广计划")][1]);
		result[result.length] = '</div><div class="queryMsgItem"><span class="queryGrayer">';
		result[result.length] = msg[3 + offset] + '</span>';
		result[result.length] = (pureData ? rowData[index][getColumnIndex("推广单元")][1] : rowData[index].data[getColumnIndex("推广单元")][1]);
		//判断是否需要添加额外信息
		if(!pureData) {
			result[result.length] = '</div><div class="queryMsgItem">';
			if (!rowData[index].failed) {//添加成功
				...
				if (offset == 6) {
					...
					result[result.length] = '</span>&nbsp&nbsp&nbsp&nbsp<a id="linkToPasvKR' + index + '">更多推荐</a><br>';
				} else {//低于最低展现价格
					result[result.length] = '</span>&nbsp&nbsp&nbsp&nbsp<span class="queryRed">最低展现价格为' + rowData[index].least;
					result[result.length] = '元&nbsp&nbsp&nbsp&nbsp</span><a id="modifyKwBid' + index;
	...
	removeQuery : function(node) {
		...
		delete QUERY_REPORT_RENDER.floatRowDatas[parseInt(node.id.match(/\d/)[0])];
		//添加判断，如果框中只剩一个词，则不能再取消。
		var len = 0,
		tmp;
		for(tmp in QUERY_REPORT_RENDER.floatRowDatas) {
			if (tmp != "length") {
				++len;
			}
		}
	...
	linkToPasvKR : function(index) {
		...
		var rowData = QUERY_REPORT_RENDER.floatRowDatas[index].data,
		word = wordEscapeHTML(rowData[getColumnIndex("搜索词")]).replace(/\r/g, '').split("\n"),
		nf = Cform(MERCURY_HOST + 'spread/InitAddWordsKR.do?userid=' + USER_ID + '&wordText=' + encode(word.replace(/'/g,"\\\&#39;"), 'post')),
		plan = rowData[getColumnIndex("推广计划")],
		unit = rowData[getColumnIndex("推广单元")];
		word = arrayDistinct(arrayRemoveBy(word));
		word = word.join(">");
		Cinput(nf, 'userid', USER_ID);
	...
	modifyKwBid : function(index) {
		...
		var rowData = QUERY_REPORT_RENDER.floatRowDatas[index].data[0].split(","),
		form = Cform('../material/wordOpAction.do?userid=' + USER_ID);
	...
	queryFloatAction : [
		...
		'<div class="qfaSelectDiv">\
			<p style="padding:0;display:inline">匹配模式：</p>&nbsp\
	 ...
	 */
	queryFloatConfrim : [
		'<div><input type="button" value="确定" id="queryFloatConfirmNeg"/>&nbsp&nbsp&nbsp&nbsp<input type="button" value="取消" id="queryFloatCancel"/></div>',
		'<div><input type="button" value="确定" id="queryFloatConfirmPos"/>&nbsp&nbsp&nbsp&nbsp<input type="button" value="取消" id="queryFloatCancel"/></div>',
	...
	queryStatus : {
		...
		"0" : '"><input class="queryUsable" id="queryActionButton" type="button"></input>\
	 ...
	 */
	repeatMsg : '<br><span class="queryGrayer qfaHeadMsg">您已选择的词存在部分重复，已进行去重处理。</span>',
...
AdaptiveSelect.prototype = {
	...
	smartHandle : function (_that, handleType){
		switch (handleType){
			...
			case 'txbtnMouseover' :
				return function(){
							_that.changeMouseOverState(_that);
						}
			case 'txbtnMouseout' :
				return function(){
							_that.changeMouseOutState(_that);
						}	
			case 'optClick':
				return function (event){
							_that.onOptClick(_that, event);
						}
			case 'optLinkMouseOver':
				return function (event){
							_that.onOptLinkMouseOver(_that, event);
						}
			case 'slectTextForbidden':
				return function (){
							_that.slectTextForbidden();
						}
		}
	},
	//替换函数
	init : function (){
		...
		//选项：点击，显示下拉选项
		textInput.onclick = this.smartHandle(_this, 'txClick');
		//选项：键盘输入，上下
		textInput.onkeydown = this.smartHandle(_this, 'kDown');
		//选项：失去焦点，恢复样式
		textInput.onblur = this.smartHandle(_this, 'txBlur');
		//下拉箭头：点击，显示下拉选项，_this.isInside修正textInput.onblur的显示逻辑
		button.onclick = this.smartHandle(_this, 'btnClick');
		//选项：鼠标移进，改变样式; 下拉箭头：鼠标移进，改变样式
		textInput.onmouseover = button.onmouseover = this.smartHandle(_this, 'txbtnMouseover');
		//选项：鼠标移出，改变样式; 下拉箭头：鼠标移出，改变样式
		textInput.onmouseout = button.onmouseout = this.smartHandle(_this, 'txbtnMouseout');
		textInput.onmousedown = this.smartHandle(_this, 'slectTextForbidden');		//禁止用户选择文本
		textInput.onselectstart = this.smartHandle(_this, 'slectTextForbidden');	//禁止用户选择文本
		//监控被替换的select状态
	...
	keydownTextInput : function (_that, event){		
		...
		var linkNum = (optGroup == null ? _that._originalSelect.selectedIndex : _that.focusLineNum);	//定位当前选项
		var mySelectText = G(_that.awSelectTextIDBase);												//模拟的select
		var selectLength = _that._originalSelect.length;											//原生Select的长度
		var optLink;	
		switch (keynum){
			case 40:	//Down
	...
	changeBlurState : function (_that) {	
	...
	changeMouseOverState : function (_that){
		addClassName(_that.awArrowIDBase, 'awArrowHover');
		_that.isInside = 1;
	},
	changeMouseOutState : function (_that){
		removeClassName(_that.awArrowIDBase, 'awArrowHover');
		_that.isInside = 0;
	},
	//下拉列表：显示
	showOptions : function (_that){			
		...
		optGroup.onmouseout = _that.smartHandle(_that,'txbtnMouseover');			//绑定鼠标移出		
		optGroup.onmousedown = _that.smartHandle(_that, 'slectTextForbidden');		//禁止用户选择文本
		optGroup.onselectstart = _that.smartHandle(_that, 'slectTextForbidden');	//禁止用户选择文本				
		
		//装载进入
		var selectArea = G(_that.awSelectIDBase);
		var optIframe = G(_that.awOptIframeIDBase);
		...
		var autoWidth = _that._originalSelect.offsetWidth;											//自适应宽度：默认包含滚动条宽度18px
		autoWidth = (autoWidth > _that.awWidth ? autoWidth : _that.awWidth + 19);					//自适应宽度：如果比指定短用指定宽度
		...
		if (autoHeight > _that.awMaxHeight + 2){													//超出指定最高需要调整，上线1px边框
			...
			optIframe.style.width = autoWidth + 2 + 'px';											//外iframe无边框需要加上div左右各1px边框宽度
			...
			optGroup.style.width = autoWidth - 18 + 'px';												
			optIframe.style.width = autoWidth - 16 + 'px';
	...
	//选项Link：点击鼠标
	onOptClick : function (_that, event){
		var event = event || window.event;
	...
	//选项Link：移动鼠标，改变样式
	onOptLinkMouseOver : function (_that, event){
		var event = event || window.event;
