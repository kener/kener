	...
	init:function (){
		...
		this.showOptions = function (){			
			...
			for(var j = 0; j < optLength; j++) {
				...
				optLink.onmousedown = this.slectTextForbidden;		//禁止用户选择文本
				optLink.onselectstart = this.slectTextForbidden;	//禁止用户选择文本
				*/
			}
			optGroup.onclick = this.onOptClick;					//绑定列表鼠标点击响应
			optGroup.onmouseover = this.onOptLinkMouseOver;		//绑定列表鼠标移动响应
			optGroup.onmousedown = this.slectTextForbidden;		//禁止用户选择文本
			optGroup.onselectstart = this.slectTextForbidden;	//禁止用户选择文本				
