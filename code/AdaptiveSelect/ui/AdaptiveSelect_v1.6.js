 ...
 * @version v1.3 修正在IE6下SelectBox.cls()后原控件自动显示，IE6下宽度获取问题
 * @version v1.4 修正在可视窗口的宽度调整问题
 * @version v1.5 绝对定位显示，修正各种因为positon:relative导致的遮挡问题	2010-06-09
 * @version v1.6 更换Iframe样式，增加z-index:20002，修正浮出窗内使用控件时最前端显示 2010-07-06
...
function AdaptiveSelect(objSelectBox, iMaxRow, iSelectWidth, iMonitorInterval){
	...
	if (Browser.IE > 0 && Browser.IE < 7) {
		this._originalSBox.cls = function(){
...
AdaptiveSelect.prototype = {
	...
	init : function (){
		...
		_this.startMonitor = function (){
			...
			if ((_this.awSelectDisplay != newDisplay)||(_this._originalSelect.style.display != 'none')){
	...
	showOptions : function (_that){			
		...
		//调整高
		var autoHeight = optGroup.scrollHeight;														//自适应高度
		...
		//修正宽度：可见视窗
		var newPos = getPosition(selectArea),
		    visibleWidth = document.documentElement.clientWidth - newPos.left;

		if (autoWidth > visibleWidth){
			autoWidth = visibleWidth - 20;
			optGroup.style.paddingRight = '1px';			
		}
		...
		//调整显示位置
		optGroup.style.left = newPos.left + 'px';
		optGroup.style.top = newPos.top + selectArea.offsetHeight + 'px';
		optGroup.style.visibility = 'visible';
		if (Browser.IE && Browser.IE < 7) {
		    //在IE6下显示iframe遮罩
           	var iframe = G('awMask');
	        
	        iframe.style.width = autoWidth + 3 + 'px';
	        if (autoHeight > _that.awMaxHeight + 2){
				iframe.style.height = _that.awMaxHeight + 2 + 'px';
			}else{
				iframe.style.height = autoHeight + 2  + 'px';
			}
     
	        iframe.style.left = optGroup.style.left;
			iframe.style.top = optGroup.style.top;
